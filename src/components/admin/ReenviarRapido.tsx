...
export interface AlvoReenvio {
  ...
  /** id do registro em compradores ou participantes */
  id: string
  nome: string
  email: string
  /** contexto para o operador conferir antes de disparar (ex.: nº do pedido) */
  contexto?: string
}
...
// A lista de templates muda pouco: busca uma vez e reaproveita na sessão.
let cacheTemplates: Tpl[] | null = null

const TEMPLATE_PADRAO: Record<string, string> = {
  compradores: 'Skip-Summit26-Send-Comprador-Email02',
  participantes: 'Skip-Summit26-Send-Participante',
}

// Reenvio individual direto da listagem, usando a mesma fila do /admin/disparo
// (cluster "individual"), então o histórico em Envios continua completo.
export default function ReenviarRapido({
  ...
  const filtrados = templates.filter((t) => {
    ...
    return publico === 'participantes' ? n.includes('participante') : n.includes('comprador')
  })
  ...
  useEffect(() => {
    ...
    const escolher = (lista: Tpl[]) => {
      const alvoNome = TEMPLATE_PADRAO[alvo.audience]
      const disponiveis = lista.filter((t) => {
        const n = (t.name || '').toLowerCase()
        if (!n.includes('skip-summit26')) return false
      ...
      const padrao = disponiveis.find((t) => t.name === alvoNome)
    ...
    pb.send('/backend/v1/admin/sendgrid/templates', {})
      ...
      .catch((e: any) => {
        setErro(
          `Não foi possível carregar os templates de e-mail: ${e?.message || 'erro desconhecido'}. Nada foi enviado.`,
        )
      })
  ...
  const enviar = async () => {
    ...
    try {
      ...
      const res: any = await pb.send('/backend/v1/admin/dispatch/enqueue', {
        method: 'POST',
        body: JSON.stringify({
          cluster: 'individual',
      ...
      setErro(`Falha ao enviar: ${e?.message || 'erro desconhecido'}. Nada foi enviado.`)
    } finally {
      setEnviando(false)
    }
  }
