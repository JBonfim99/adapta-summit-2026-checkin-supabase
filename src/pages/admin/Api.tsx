...
import { Download, KeyRound, Globe } from 'lucide-react'

const API_KEY = '$EXTERNAL_API_KEY'
const BASE_URL = `${String(import.meta.env.VITE_SUPABASE_URL ?? 'http://127.0.0.1:54321').replace(/\/$/, '')}/functions/v1/public-api`
...
const ENDPOINTS = [
  ...
  },
  {
    ...
    caminho: '/backend/v1/external/compradores',
    descricao:
      'Cria (ou atualiza, se o e-mail já existir) um comprador com N ingressos GOLD ou PLATINUM. Dispara automaticamente o e-mail de acesso.',
    params: [
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in-up pb-12">
      ...
      <Card className="border-none shadow-sm">
        <CardHeader>
          ...
          <CardDescription>
            Toda chamada precisa do header <code className="text-xs">X-Api-Key</code>. A chave fica
            somente nos secrets do Supabase e deve ser rotacionada pela equipe responsável.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex items-center gap-2 bg-slate-50 border rounded-lg px-3 py-2">
            <code className="text-sm flex-1 truncate">Definida no secret EXTERNAL_API_KEY</code>
          </div>
