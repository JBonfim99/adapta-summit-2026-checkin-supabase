// ---------------------------------------------------------------------------
// DIAGNÓSTICO só-leitura do lote de conversão GOLD->PLATINUM (migration
// 0034_upgrade_lote_gold_platinum). Reprocessa as mesmas 60 linhas do CSV
// (mesmo casamento por email -> CPF) e devolve o estado ATUAL de cada uma, sem
// alterar nada. Feito pra conferir o resultado depois de aplicar a migration.
// Pode apagar este arquivo depois de conferir — não é usado em nenhum fluxo.
// ---------------------------------------------------------------------------
...
routerAdd(
  'GET',
  '/backend/v1/admin/upgrade-lote-status',
  (e) => {
    const out = {
      resumo: {
        ...
        criados: 0,
        convertidos: 0,
        convertidos_precisa_inac_edit: 0,
        nao_convertido_ainda_tem_gold: 0,
    ...
    for (const row of ROWS) {
      ...
      if (!comprador) {
        out.resumo.comprador_nao_encontrado++
        out.linhas.push({
          nome: row.nome,
          email: email,
      ...
      const ticketsInfo = tickets.map((t) => ({
        ...
        status_webhook: t.getString('status_webhook'),
      }))

      let classificacao = 'indefinido'
      const criadoNesteLote = ticketsInfo.find((t) => t.pedido_id === row.transacao)
      const precisaEdit = ticketsInfo.find(
        (t) => (t.origem || '').indexOf('pending-inac-edit') !== -1,
      )
      const aindaTemGold = ticketsInfo.some((t) => t.tipo_ingresso === 'GOLD')
      const temPlatinum = ticketsInfo.some((t) => t.tipo_ingresso === 'PLATINUM')

      if (criadoNesteLote) {
        classificacao = 'criado'
      } else if (precisaEdit) {
        classificacao = 'convertido_precisa_inac_edit'
      } else if (aindaTemGold) {
        classificacao = 'nao_convertido_ainda_tem_gold'
      } else if (temPlatinum) {
